...
export default function CreateCommunity({ onClose, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // setCommunityData= tag2.split(',').map(tag2=>tag2.trim());
    // setCommunityData((prevData) => ({ ...prevData, [tags]:tag2.split(',').map(tag2=>tag2.trim()) }));
    try {
      // Add your logic to handle form submission
      const token = localStorage.getItem('Token');
      const response = await fetch('https://campusconnectbackend.onrender.com/api/v1/community/create', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          tags:tag2.split(',').map(tag2=>tag2.trim()),
        }),
      });
...
//       formData.append('tags', tag2.split(',').map(tag2 => tag2.trim()).join(','));
